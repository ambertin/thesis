...
``In spite of some pioneering effort such as (Howe {[}12{]}),
...
in professional schools such as the iSchools, probably because the
students are usually considered to be non-research oriented.''
(\url{https://www.mitpressjournals.org/doi/pdf/10.1162/dint_a_00053})
\begin{itemize}
\item
  Natural place to bring awareness/education about reproducibility is
  early on in data science educational pipelines, either in
  undergraduate or graduate programs
  (\url{https://arxiv.org/abs/1401.3269})
\item
  Teaching transparency tools and encouraging students to produce
  reproducible work early on can help lessen the burden on journals in
  the long run by increasing the number (and percentage) of researchers
  doing reproducible work.
\item
  Teaching early gives students the opportunity to learn when something
  is reproducible and when it's not, as well as methods for achieving
  reproducibility.
\item
  It gives students the tools to add knowledge to their field in the
  best way possible (Bringing the Gold Standard into the Classroom:
  Replication in University Teaching - Nicole Janz)
\item
  Practicing from the start makes it so that reproducibility comes
  naturally and easily -- without much effort -- rather than having to
  struggle through it every time
\item
  Grading gives an incentive for students to pay attention, learn, and
  absorb the information better than if they tried to learn it elsewhere
\item
  There have been some efforts to introduce reproducibility in
  scientific education:
\end{itemize}
See \url{https://escholarship.org/uc/item/90b2f5xh} (RMarkdown - Ben
Baumer), KING, GARY. (2006) Publication. PS, Political Science \&
Politics 39 (1): 119--125. (forcing students to re-run the results of
studies as part of class!), Cambridge Replication Workshop (described in
Janz article on pg 399), Berkeley course where students each take a
different piece of the pipeline and have to make sure their piece
connects with the next piece
(\url{https://berkeleysciencereview.com/2014/06/reproducible-collaborative-data-science/})

Pg 398 of the Nicole Janz article has a lot more examples!
...
\texttt{packrat} (Ushey, McPherson, Cheng, Atkins, \& Allaire (2018)) is
focused on dependencies, creating a packaged folder containing a project
as well as all of its dependencies so that projects dependent on
lesser-used packages can be easily shared across computers.

\texttt{drake} (OpenSci (2020)) analyzes workflows, skips steps where
results are up to date, and provides evidence that results match the
...
\textbf{on r packages} Many of these packages are narrow, with each
effectively addressing a small component of reproducibility: file
structure, modularization of code, version control, etc. These packages
often succeed in their area of focus, but at the cost of accessibility
to a wider audience. Their functions are often quite complex to use, and
many steps must be completed to achieve the required reproducibility
goal. This cumbersome nature means that most reproducibility packages
currently available are not easily accessible to users near the
beginning of their \texttt{R} journey, nor particularly useful to those
...
** on continuous integration ** However, while these tools can be
useful, they are generalized so as to be useful to the widest audience.
As a result, their checks are not designed to be \texttt{R}-specific,
which makes them sub-optimal for users looking to address
reproducibility issues involving features specific to the \texttt{R}
programming language, such as package installation and seed setting.
...
\end{Shaded}
\begin{verbatim}
-- The available checks in `fertile` are
\end{verbatim}
\begin{verbatim}
...
\end{Shaded}
\begin{verbatim}
-- Analysis of reproducibility for proje
\end{verbatim}
\begin{verbatim}
...
2 citrate_v_time.p~ png      186.2K image/png                         
3 proteins_v_time.~ png     379.78K image/png                         
...
# A tibble: 6 x 4
  ...
  <chr>       <chr>                       <chr>    <dttm>             
1 package:re~ <NA>                        base::r~ 2020-09-12 18:45:28
2 package:th~ <NA>                        base::r~ 2020-09-12 18:45:28
3 package:th~ <NA>                        base::l~ 2020-09-12 18:45:28
4 package:pu~ <NA>                        base::l~ 2020-09-13 00:52:15
5 package:fo~ <NA>                        base::l~ 2020-09-13 00:52:15
6 project_mi~ /Users/audreybertin/Docume~ readr::~ 2020-09-13 00:52:15
\end{verbatim}
...
\end{Shaded}
\begin{verbatim}
/var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/RtmpcAu5n8/project_miceps
\end{verbatim}
\begin{Shaded}
\begin{Highlighting}[]
...
\end{Shaded}
\begin{verbatim}
/var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/RtmpcAu5n8/project_miceps/Blot_data_updated.csv
/var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/RtmpcAu5n8/project_miceps/CS_data_redone.csv
/var/folders/v6/f62qz88s0sd5n3yqw9d8sb300000gn/T/RtmpcAu5n8/project_miceps/Estrogen_Receptors.docx
\end{verbatim}
