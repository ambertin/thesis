...
Journals that already have designated reproducibility editors, such as *Biostatistics* and the *Journal of the American Statistical Association*, could also benefit from `fertile` (@journal-biostats, @jasa-rep-editors). The integration of `fertile` could speed up the review process, allowing the editors to review more papers for reproducibility adherence in a shorter time.
...
Although the process may entail some confusion and troubleshooting at first, even those brand new to `R` could succeed in overcoming these barriers in only a few days of class. As a result, `fertile` could provide professors with an opportunity to teach reproducibility concepts in introductory level courses. 
...
`fertile` can also provide benefits in other domains. Although not an exhaustive list, some of the potential uses of the software are: 
...
`fertile` is not only helpful in the specific circumstances described in the previous few sections. It can also provide benefits to `R` users more generally. All users, no matter their end goal, can see positive changes by focusing more on the reproducibility of their work. 

`fertile` promotes the use of an R project format, which can help simplify the process of referencing and sharing files. It also encourages users to use an R package structure, which can make it easier to share functions and data that come alongside projects. 

More generally, reproducibility makes it easier for users to maintain and share their code. If a user leaves a project for a while and then comes back to try and run it, dependency reports and other documentation can simplify the process of ensuring that everything compiles smoothly. And if a user has to move their work to another computer or share it with a friend or colleague, reproducibility ensures that it will still run successfully.
...
Our goal was to determine whether students who had `fertile` installed and loaded on their RStudio applications would learn more about reproducibility throughout the course than students who did not, or whether there was instead no difference or even a negative effect.
...
```{r, echo = FALSE, message = FALSE, fig.cap = 'Scatterplot: Previous Coursework vs Total Reproducibility Test Score, Fall 2020', fig.height = 3, fig.width = 5}
...
There is the additional limitation that although this experimental structure tests the pure educational power of `fertile`, it does not accurately represent the ways in which `fertile` would be used in practice.
